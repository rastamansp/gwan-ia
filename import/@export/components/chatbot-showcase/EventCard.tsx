...
export const EventCard: React.FC<EventCardProps> = ({ event, onEventClick }) => {
  ...
  const getEventLink = (): string | undefined => {
    ...
    return eventData.eventLink || eventData.event_link || eventData.link || eventData.url || eventData.eventUrl;
  };
  ...
  return (
    <div 
      ...
      className={`bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden transition-all ${
        hasLink ? 'cursor-pointer hover:shadow-md hover:border-blue-300' : 'cursor-default'
      }`}
    ...
    >
      ...
      <div className="p-3">
        <h4 className="text-sm font-semibold text-gray-900 mb-1 line-clamp-1">{event.title}</h4>
        {event.description && (
        ...
        <div className="space-y-1">
          <div className="flex items-center text-gray-600 text-xs">
            ...
            <span className="truncate">{formatDate(event.date)} às {formatTime(event.date)}</span>
          </div>
          <div className="flex items-center text-gray-600 text-xs">
            <MapPin className="h-3 w-3 mr-1 flex-shrink-0" />
            <span className="truncate">
              {event.location}{event.address ? `, ${event.address}` : ''}, {event.city}{event.state ? ` - ${event.state}` : ''}
            </span>
          </div>
          {(event.maxCapacity !== undefined || event.soldTickets !== undefined) && (
            <div className="flex items-center text-gray-600 text-xs">
              <Users className="h-3 w-3 mr-1 flex-shrink-0" />
              <span>
                {event.soldTickets ?? 0} de {event.maxCapacity ?? 0} ingressos vendidos
              </span>
            </div>
          )}
        </div>
