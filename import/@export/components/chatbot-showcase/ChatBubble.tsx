...
export const ChatBubble = ({ type, content, sender, timestamp, caption, duration, suggestions, events, onSuggestionClick }: ChatBubbleProps) => {
  ...
  if (type === "event_list" && events && events.length > 0) {
    ...
    return (
      <div className={`max-w-[85%] ${isSent ? "ml-auto" : "mr-auto"} mb-2 animate-fade-in`}>
        <div className={`${bubbleClass} px-4 py-3 shadow-sm`}>
          <EventCard event={event} />
          <span className="text-xs text-gray-600 mt-2 block text-right">{timestamp}</span>
        </div>
      </div>
    );
  }
