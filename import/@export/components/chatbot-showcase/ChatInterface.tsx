...
export const ChatInterface = ({ 
  ...
  const allMessages: JourneyMessage[] = journeyMessages || (realTimeMessages.length > 0 ? realTimeMessages : conversation.messages as JourneyMessage[]);
  ...
  const isJourneyMode = !!journeyMessages;
  ...
  const handleSendMessage = useCallback(async (e: React.FormEvent, messageText?: string) => {
    ...
    try {
      ...
      const response = await chatRepository.sendMessage({ message: userMessageText });
      ...
      const events = (response.formattedResponse?.data?.rawData || response.formattedResponse?.data?.items) as EventItem[] | undefined;
      
      // Usar o answer completo sem processamento
      const answerText = response.formattedResponse?.answer || response.answer;
      ...
      if (responseType === "event_list" && events && events.length > 0) {
        ...
        const outroMessage: JourneyMessage = {
          ...
          sender: "mentor",
          type: "text",
          content: "Se precisar de mais informações sobre algum evento específico, é só avisar!",
          timestamp: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
        };
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <form onSubmit={handleSendMessage} className="bg-white px-4 py-3 flex items-center gap-2 border-t border-gray-200 flex-shrink-0">
