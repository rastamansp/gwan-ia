...
import jaiminhoJourneysData from '../../data/chat/jaiminho-journeys.json';
...
interface UseJaiminhoInteractionsResult {
  journeys: Journey[];
...
export const useJaiminhoInteractions = (): UseJaiminhoInteractionsResult => {
  const [selectedJourney, setSelectedJourney] = useState<Journey | null>(null);

  const journeys = useMemo(() => {
    const data = jaiminhoJourneysData as JourneysData;
    return data.journey || [];
  }, []);
