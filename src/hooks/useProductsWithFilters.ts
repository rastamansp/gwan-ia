...
import { FeaturedProduct, fetchProductsWithFilters } from '../services/product.service';
...
export const useProductsWithFilters = (initialParams: SearchParams = {}) => {
  ...
  const searchProducts = async (params: SearchParams = {}) => {
    try {
      ...
      const result: PaginatedResponse<FeaturedProduct> = await fetchProductsWithFilters(newParams);
      ...
      if (result.data.products.length > 0) {
        console.log('First product details:', {
          ...
          currentPrice: result.data.products[0].currentPrice,
          originalPrice: result.data.products[0].originalPrice,
          promotionalPrice: result.data.products[0].promotionalPrice,
          formattedCurrentPrice: result.data.products[0].formattedCurrentPrice,
          formattedOriginalPrice: result.data.products[0].formattedOriginalPrice,
